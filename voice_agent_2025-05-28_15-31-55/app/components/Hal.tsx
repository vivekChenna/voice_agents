...
const draw = (ctx: Context, shape: ShapeRef, last: number, now: number): void => {
  ...
  // Draw a single elegant ampersand in the center
  const ampRadius = maxRadius * 0.7 * lerp(1, FOCUS_SIZE_MULTIPLIER, easeInOutQuad(shape.current.focus));
  const ampFontSize = Math.max(ampRadius, 10);
  ctx.save();
  ctx.font = `normal ${ampFontSize}px Arial, Helvetica, sans-serif`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.lineWidth = 2;
  ctx.shadowColor = 'rgba(0,0,0,0.18)';
  ctx.shadowBlur = 8;
  // Animated gradient for ampersand
  const ampGradient = ctx.createLinearGradient(center.x - ampRadius, center.y - ampRadius, center.x + ampRadius, center.y + ampRadius);
  ampGradient.addColorStop(0, '#13ef93');
  ampGradient.addColorStop(1, '#149afbcc');
  ctx.strokeStyle = ampGradient;
  ctx.fillStyle = 'rgba(255,255,255,0.18)';
  ctx.fillText('&', center.x, center.y);
  ctx.strokeText('&', center.x, center.y);
  ctx.restore();
